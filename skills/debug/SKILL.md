...
5. Form a hypothesis: "The failure is caused by X because Y."
6. Add a minimal reproduction — reduce to the smallest failing case.
7. Inspect relevant code: read the file, find the function, trace the data flow.
8. Check recent changes: `git log --oneline -10` — did anything change that could cause this?

### แก้ bug (Fix)

9. Apply the minimal fix — change only what's needed.
10. Avoid fixing symptoms — address the root cause.
11. Run the failing test: must now PASS.
12. Run the full suite: must still pass.

### ตรวจยืนยัน (Verify)

13. Run full test suite: `make test-unit`
14. If integration tests exist: `make test-int`
15. Confirm no regressions.
