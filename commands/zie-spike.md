...
4. **Summarize** — in `spike-<slug>/FINDINGS.md`:
   ...
   - If proceeding → `/zie-backlog` to capture as a real backlog item.

5. **Done** — print:
   ```
   Spike complete: spike-<slug>/FINDINGS.md
   Next: /zie-backlog "<slug>" to promote, or discard spike-<slug>/ if abandoned.
   ```
...
- Promote findings via `/zie-backlog` if the spike validates an approach.
