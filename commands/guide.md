...
| spec-unapproved | Spec file exists but `approved: true` absent | Run `Skill('zie-framework:spec-review')` then `python3 hooks/approve.py <spec-path>` |
...
2. **Review** — after writing spec, run `Skill('zie-framework:spec-review')` to validate
