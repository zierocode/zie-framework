...
BLOCKS = [
    ...
    (
        r"git\s+push\b(?!.*--tags).*\borigin\s+main\b",
        "Direct push to main blocked — use 'make release NEW=x.y.z' instead",
    ),
    (
        r"git\s+push\b(?!.*--tags).*\borigin\s+master\b",
        "Direct push to master blocked — use 'make release NEW=x.y.z' instead",
    ),
    # Hard reset
...
INJECTION_BLOCKS = [
    ...
    (r"(pretend|act)\s+(you\s+are|as\s+if)\b", "role-play pattern blocked — command contains prompt-injection attempt"),
    (
    ...
    (r"return\s+ALLOW", "instruction-injection pattern blocked — command attempts to force ALLOW response"),
    (r"output\s+BLOCK", "instruction-injection pattern blocked — command attempts to manipulate safety output"),
    (r"\bsystem:\s", "system-injection pattern blocked — command contains fake system prompt"),
]
...
WARNS = [
    ...
    (r"alembic\s+downgrade\b", "Alembic downgrade detected — verify this won't lose production data"),
]
