...
EXCLUDE = {"node_modules", ".git", "build", "dist", ".next", "__pycache__", "coverage", "zie-framework", ".zie"}
...
parser.add_argument(
    "--now", action="store_true", help="Print current hash to stdout (default; accepted for compatibility)"
)
parser.add_argument(
    "--check", action="store_true", help="Compare stored vs current hash; print drift warning if mismatch"
)
...
def compute_hash(root: Path) -> str:
    dirs = sorted(
        ...
        for p in root.rglob("*")
        if p.is_dir() and not any(ex in p.parts for ex in EXCLUDE) and str(p.relative_to(root)) not in EXCLUDE_PATHS
    )
    counts = sorted(f"{d}:{len(list((root / d).iterdir()))}" for d in dirs)
    configs = ""
...
def _compute_max_mtime(root: Path) -> float:
    ...
    try:
        mtimes = [p.stat().st_mtime for p in root.rglob("*.md") if "zie-framework" not in p.parts]
...
if args.check:
    try:
        ...
        cache.set("knowledge_mtime", "knowledge-hash-check", current, ttl=3600, invalidation="session")
        if current != stored:
            print("[zf] Knowledge drift detected since last session — run /resync to update project context")
