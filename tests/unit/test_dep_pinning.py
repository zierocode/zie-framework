...
class TestDepPinning:
    ...
    def test_all_deps_use_compatible_release_pinning(self):
        ...
        dep_lines = [line for line in lines if line.strip() and not line.strip().startswith("#")]
        assert dep_lines, "requirements-dev.txt has no dependency lines"
        bad_lines = [line for line in dep_lines if not re.search(r"~=\d", line)]
        assert bad_lines == [], (
    ...
    def test_no_exact_pin(self):
        ...
        dep_lines = [line for line in content.splitlines() if line.strip() and not line.strip().startswith("#")]
        exact_pins = [line for line in dep_lines if re.search(r"==\d", line)]
        assert exact_pins == [], f"Exact pins (==) found; use ~= instead: {exact_pins}"

    def test_no_lower_bound_only_pin(self):
        content = REQ_FILE.read_text()
        dep_lines = [line for line in content.splitlines() if line.strip() and not line.strip().startswith("#")]
        lower_bound_only = [line for line in dep_lines if re.search(r">=\d", line)]
        assert lower_bound_only == [], f"Lower-bound-only pins (>=) found; use ~= instead: {lower_bound_only}"
