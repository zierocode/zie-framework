...
class TestInlineGuidanceBlock:
    ...
    def test_per_task_tdd_loop_skill_present(self):
        text = read_cmd()
        assert "Skill(zie-framework:tdd-loop)" in text, "Skill(zie-framework:tdd-loop) must appear in zie-implement.md"
    ...
    def test_tdd_deep_conditional_absent(self):
        ...
        text = read_cmd()
        assert "tdd: deep" not in text, "tdd: deep conditional must be removed from zie-implement.md"


class TestParallelByDefault:
    def test_parallel_execution_documented(self):
        text = read_cmd()
        assert "background" in text.lower() or "parallel" in text.lower() or "concurrent" in text.lower(), (
            "zie-implement.md must document parallel/background execution"
        )

    def test_sequential_task_loop_present(self):
        text = read_cmd()
        assert "Task Loop" in text or "task loop" in text.lower(), "Sequential task loop structure must be documented"
