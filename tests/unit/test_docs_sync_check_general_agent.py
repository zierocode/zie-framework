...
class TestDocsSyncCheckGeneralAgent:
    def test_release_no_docs_sync_check_plugin_agent(self):
        ...
        assert (
            ...
            and "subagent_type='zie-framework:docs-sync'" not in text
    ...
    def test_release_uses_skill_for_docs_sync(self):
        ...
        text = RELEASE_PATH.read_text()
        assert "docs-sync" in text, "zie-release must reference docs-sync"

    def test_retro_uses_skill_for_docs_sync(self):
        """zie-retro uses Skill(zie-framework:docs-sync) for docs sync."""
        text = RETRO_PATH.read_text()
        assert "Skill(zie-framework:docs-sync)" in text, "zie-retro must use Skill(zie-framework:docs-sync)"
    ...
    def test_docs_sync_inline_instructions_in_retro(self):
        ...
        text = RETRO_PATH.read_text()
        assert "docs-sync" in text, "zie-retro must reference docs-sync"
...
class TestDocsSyncCheckProjectMd:
    ...
    def test_skill_reads_project_md(self):
        ...
        assert "PROJECT.md" in self._skill(), "docs-sync SKILL.md must mention PROJECT.md"

    def test_skill_has_step_3b(self):
        """Skill must contain a Step 3b block."""
        assert "3b" in self._skill(), "docs-sync SKILL.md must have a Step 3b"
    ...
    def test_verdict_has_project_md_stale(self):
        ...
        assert "project_md_stale" in self._skill(), "docs-sync verdict JSON must include project_md_stale"
    ...
    def test_verdict_has_missing_and_extra_fields(self):
        ...
        skill = self._skill()
        assert "missing_from_project_md" in skill, "Verdict must include missing_from_project_md"
        assert "extra_in_project_md" in skill, "Verdict must include extra_in_project_md"
