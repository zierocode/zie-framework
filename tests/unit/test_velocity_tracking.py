...
class TestVelocityTracking:
    ...
    def test_velocity_output_line_present(self):
        text = read_status_command()
        assert "Velocity" in text, "zie-status.md must contain a Velocity output line"

    def test_graceful_fallback_present(self):
        text = read_status_command()
        assert "not enough releases yet" in text, "zie-status.md must contain graceful fallback for <2 semver tags"
