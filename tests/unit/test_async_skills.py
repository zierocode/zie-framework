...
class TestAsyncSkillPatterns:
    ...
    def test_zie_retro_writes_adrs_inline(self):
        ...
        assert "Write" in text and "ADR" in text, "zie-retro.md must have inline Write for ADR files"
        assert "run_in_background" not in text, "zie-retro.md must NOT use run_in_background for ADR/ROADMAP writes"
        assert "Agent(" not in text, "zie-retro.md ADR/ROADMAP section must not spawn agents"
    ...
    def test_zie_release_uses_background_execution(self):
        ...
        assert "run_in_background" in text, "zie-release.md must use run_in_background=True for parallel gates"
        assert "make test-int" in text, "zie-release.md must invoke make test-int for Gate 2"
        assert "Agent(" not in text, "zie-release.md must use inline Bash, not Agent() spawning"
    ...
    def test_zie_implement_uses_taskcreate_for_verify(self):
        ...
        assert "TaskCreate" in text, "zie-implement.md must use TaskCreate for verify step"
        assert "verify" in text.lower(), "zie-implement.md must reference verify"
    ...
    def test_fallback_handling_present(self):
        ...
        assert "docs-sync unavailable" in release, "zie-release.md must document docs-sync unavailable skip message"
