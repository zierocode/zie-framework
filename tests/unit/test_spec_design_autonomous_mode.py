...
class TestSpecDesignAutonomousMode:
    def test_autonomous_mode_argument_documented(self):
        assert "autonomous" in text(), "spec-design must document autonomous as a valid $ARGUMENTS[1] value"

    def test_skips_interactive_steps_in_autonomous(self):
        t = text().lower()
        assert "autonomous" in t and ("skip" in t or "direct" in t), "autonomous mode must skip interactive steps"
    ...
    def test_inline_reviewer_call_in_autonomous(self):
        t = text().lower()
        assert "autonomous" in t and "inline" in t, "autonomous mode must call spec-review inline"
