...
bandit = pytest.importorskip("bandit", reason="bandit not installed — skip bandit CI tests on Python 3.14+")
...
class TestBanditSast:
    def test_bandit_is_importable(self):
        ...
        assert result.returncode == 0, "bandit is not installed — run: pip install bandit>=1.7\n" + result.stderr

    @pytest.mark.skipif(sys.version_info >= (3, 14), reason="bandit not available on Python 3.14+")
    def test_bandit_hooks_exits_clean(self):
        ...
        assert result.returncode == 0, "bandit found issues in hooks/:\n" + result.stdout + result.stderr
    ...
    def test_make_lint_bandit_target_exists(self):
        ...
        assert "lint-bandit:" in self._makefile_content(), "lint-bandit target not found in Makefile or Makefile.local"
    ...
    def test_make_lint_calls_lint_bandit(self):
        ...
        for line in content.splitlines():
            if line.startswith("lint:") or line.startswith("lint "):
                assert "lint-bandit" in line, f"lint target does not call lint-bandit: {line!r}"
                break
        else:
...
class TestPreCommitBanditIntegration:
    def test_pre_commit_calls_lint_bandit(self):
        ...
        assert "lint-bandit" in content, "pre-commit hook does not call lint-bandit"
    ...
    def test_pre_commit_has_bandit_install_guard(self):
        ...
        assert "command -v bandit" in content or "pip install bandit" in content, (
