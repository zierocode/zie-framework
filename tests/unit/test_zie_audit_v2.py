...
class TestPhase1ContextBundle:
    ...
    def test_manifest_detection_is_generic(self):
        ...
        assert found >= 2, \
            "Skill must detect manifests from multiple ecosystems (generic)"
...
class TestPhase2ExternalResearch:
    ...
    def test_external_research_uses_websearch(self):
        ...
        assert "WebSearch" in text, \
            "External Research must use WebSearch"
...
class TestPhase2DimensionConsolidation:
    def test_dependency_health_covered(self):
        ...
        assert "depend" in text or "outdated" in text or "license" in text, \
            "Skill must cover Dependency Health"

    def test_performance_dimension_covered(self):
        text = read_skill().lower()
        assert "performance" in text or "n+1" in text or "blocking" in text, \
            "Skill must cover Performance"

    def test_observability_dimension_covered(self):
        text = read_skill().lower()
        assert "observability" in text or "health check" in text or "metric" in text, \
            "Skill must cover Observability"
...
class TestPhase3InlineSynthesis:
    ...
    def test_synthesis_no_websearch(self):
        ...
        p4 = _phase_text(4)
        assert "WebSearch" not in p4 or "no WebSearch" in p4, \
            "Phase 4 (Synthesis) must not use WebSearch"
...
class TestPhase4BatchBacklog:
    def test_batch_prompt_not_one_by_one(self):
        text = read_skill().lower()
        assert "all" in text or "batch" in text, \
            "Skill must offer batch backlog addition, not one-by-one"

    def test_critical_and_high_offered(self):
        text = read_skill()
        assert "CRITICAL" in text or "critical" in text.lower(), \
            "Skill must surface CRITICAL findings for backlog"
