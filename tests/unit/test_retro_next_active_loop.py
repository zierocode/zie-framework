...
class TestRetroNextActiveLoop:
    ...
    def test_zie_plan_prompt_present(self):
        assert "/plan" in _text(), \
            "zie-retro.md must contain a '/plan' prompt in the Suggest next step"

    def test_empty_backlog_fallback_present(self):
        assert "/backlog" in _text(), \
            "zie-retro.md must contain the empty-backlog fallback referencing /backlog"
