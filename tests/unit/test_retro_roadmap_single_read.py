...
class TestRoadmapSingleRead:
    def test_preflight_binds_roadmap_raw(self):
        src = CMD.read_text()
        assert "roadmap_raw" in src, "Pre-flight must bind roadmap_raw variable"
    ...
    def test_roadmap_raw_defined_before_steps(self):
        ...
        assert raw_pos < preflight_end, "roadmap_raw must be bound before ## Steps section"

    def test_done_write_does_not_re_read_roadmap(self):
        src = CMD.read_text()
        section_start = src.find("บันทึก ADRs + อัปเดต ROADMAP")
        assert section_start != -1, "ADRs + ROADMAP update section must exist"
        ...
        assert "Read `zie-framework/ROADMAP.md`" not in section, (
            "Done-write section must not re-read ROADMAP.md — use pre-flight binding"
        )
        # The section uses pre-bound data (roadmap_raw from pre-flight)
        # rather than re-reading the file inline
    ...
    def test_done_rotation_uses_roadmap_raw(self):
        ...
        assert "roadmap_raw" in section, "Done-rotation must use roadmap_raw binding"
