...
class TestSpecReviewerForkContext:
    def test_has_context_fork(self):
        ...
        assert "context: fork" in fm, \
            "spec-review frontmatter must contain 'context: fork'"

    def test_has_agent_explore(self):
        fm = read_frontmatter("spec-review")
        assert "agent: Explore" in fm, \
            "spec-review frontmatter must contain 'agent: Explore'"

    def test_has_allowed_tools(self):
        fm = read_frontmatter("spec-review")
        assert "allowed-tools:" in fm, \
            "spec-review frontmatter must contain 'allowed-tools:'"

    def test_allowed_tools_read(self):
        fm = read_frontmatter("spec-review")
        assert "Read" in fm, \
            "spec-review allowed-tools must include Read"

    def test_allowed_tools_grep(self):
        fm = read_frontmatter("spec-review")
        assert "Grep" in fm, \
            "spec-review allowed-tools must include Grep"

    def test_allowed_tools_glob(self):
        fm = read_frontmatter("spec-review")
        assert "Glob" in fm, \
            "spec-review allowed-tools must include Glob"

    def test_no_bash_in_allowed_tools(self):
        fm = read_frontmatter("spec-review")
        assert "Bash" not in fm, \
            "spec-review is Explore agent — Bash must not appear in frontmatter"
...
class TestPlanReviewerForkContext:
    def test_has_context_fork(self):
        ...
        assert "context: fork" in fm, \
            "plan-review frontmatter must contain 'context: fork'"

    def test_has_agent_explore(self):
        fm = read_frontmatter("plan-review")
        assert "agent: Explore" in fm, \
            "plan-review frontmatter must contain 'agent: Explore'"

    def test_has_allowed_tools(self):
        fm = read_frontmatter("plan-review")
        assert "allowed-tools:" in fm, \
            "plan-review frontmatter must contain 'allowed-tools:'"

    def test_allowed_tools_read(self):
        fm = read_frontmatter("plan-review")
        assert "Read" in fm, \
            "plan-review allowed-tools must include Read"

    def test_allowed_tools_grep(self):
        fm = read_frontmatter("plan-review")
        assert "Grep" in fm, \
            "plan-review allowed-tools must include Grep"

    def test_allowed_tools_glob(self):
        fm = read_frontmatter("plan-review")
        assert "Glob" in fm, \
            "plan-review allowed-tools must include Glob"

    def test_no_bash_in_allowed_tools(self):
        fm = read_frontmatter("plan-review")
        assert "Bash" not in fm, \
            "plan-review is Explore agent — Bash must not appear in frontmatter"
...
class TestImplReviewerForkContext:
    def test_has_context_fork(self):
        ...
        assert "context: fork" in fm, \
            "impl-review frontmatter must contain 'context: fork'"

    def test_has_agent_general_purpose(self):
        fm = read_frontmatter("impl-review")
        assert "agent: general-purpose" in fm, \
            "impl-review frontmatter must contain 'agent: general-purpose'"

    def test_not_agent_explore(self):
        fm = read_frontmatter("impl-review")
        assert "agent: Explore" not in fm, \
            "impl-review must use general-purpose agent, not Explore"

    def test_has_allowed_tools(self):
        fm = read_frontmatter("impl-review")
        assert "allowed-tools:" in fm, \
            "impl-review frontmatter must contain 'allowed-tools:'"

    def test_allowed_tools_read(self):
        fm = read_frontmatter("impl-review")
        assert "Read" in fm, \
            "impl-review allowed-tools must include Read"

    def test_allowed_tools_grep(self):
        fm = read_frontmatter("impl-review")
        assert "Grep" in fm, \
            "impl-review allowed-tools must include Grep"

    def test_allowed_tools_glob(self):
        fm = read_frontmatter("impl-review")
        assert "Glob" in fm, \
            "impl-review allowed-tools must include Glob"

    def test_allowed_tools_bash(self):
        fm = read_frontmatter("impl-review")
        assert "Bash" in fm, \
            "impl-review allowed-tools must include Bash (general-purpose agent needs shell access)"
