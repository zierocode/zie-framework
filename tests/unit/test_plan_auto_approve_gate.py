...
class TestPlanAutoApprove:
    ...
    def test_auto_approve_on_reviewer_approved(self):
        ...
        assert (
            "auto" in text.lower()
            or "automatically" in text.lower()
            or ("APPROVED" in text and "frontmatter" in text.lower())
    ...
    def test_roadmap_move_present(self):
        ...
        text = cmd_text()
        assert "Ready" in text and "Next" in text, "zie-plan must still move item from Next to Ready"
