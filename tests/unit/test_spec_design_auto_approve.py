...
class TestSpecDesignAutoApprove:
    def test_no_ask_user_to_review_step(self):
        ...
        text = skill_text()
        assert "Ask user to review" not in text, "spec-design must not ask user to review after spec-review APPROVED"

    def test_record_approval_step_present(self):
        """Frontmatter is written automatically when spec-review returns APPROVED."""
        text = skill_text()
        assert "Record approval" in text, "spec-design must still auto-write frontmatter on APPROVED"
