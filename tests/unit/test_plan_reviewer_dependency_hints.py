...
class TestPlanReviewerDependencyHints:
    ...
    def test_dependency_scan_item_present(self):
        ...
        assert "**Dependency hints**" in text, "Phase 2 must contain a Dependency hints item"

    def test_suggestion_output_format_exact(self):
        text = SKILL_FILE.read_text()
        assert ("Tasks N and M appear independent — consider `<!-- depends_on: -->` for parallel execution") in text, (
            "Suggestion output format does not match required exact text"
        )

    def test_item_is_advisory_not_blocking(self):
        text = SKILL_FILE.read_text()
        assert "suggestion" in text.lower(), "Dependency hints item must be labelled as a suggestion (not an error)"
    ...
    def test_file_map_heuristic_present(self):
        text = SKILL_FILE.read_text()
        assert "file → tasks" in text or "file→tasks" in text or "file → [task" in text or "file-map" in text.lower(), (
            "Step 10 must describe the file-map heuristic"
        )

    def test_pair_check_removed(self):
        text = SKILL_FILE.read_text()
        assert "for each pair" not in text.lower(), "Step 10 must not instruct pair-wise checking"
    ...
    def test_step10_map_build_before_flag(self):
        ...
        map_mention = "file→tasks" in step10_text or "file → tasks" in step10_text or "file → [task" in step10_text
