...
class TestProjectMdHub:
    def test_project_md_exists(self):
        assert os.path.isfile(os.path.join(ZIE, "PROJECT.md")), "zie-framework/PROJECT.md must exist"

    def test_project_md_size_limit(self):
        with open(os.path.join(ZIE, "PROJECT.md")) as f:
            lines = f.readlines()
        assert len(lines) <= 80, f"PROJECT.md must be ≤ 80 lines (hub stays lean), found {len(lines)}"

    def test_project_md_has_command_table(self):
        content = read("zie-framework/PROJECT.md")
        assert "## Commands" in content, "PROJECT.md must have a ## Commands section"
...
class TestSpokes:
    ...
    def test_context_md_exists(self):
        assert os.path.isfile(os.path.join(ZIE, "project", "context.md")), "zie-framework/project/context.md must exist"
...
class TestInitTemplates:
    ...
    def test_project_template_has_placeholder(self):
        ...
        assert "{{project_name}}" in content, "PROJECT.md.template must use {{project_name}} placeholder"
    ...
    def test_init_references_project_md(self):
        ...
        assert "PROJECT.md" in content, "/init must reference PROJECT.md creation"


class TestRetroKnowledgeSync:
    def test_retro_has_knowledge_sync_section(self):
        content = read("commands/retro.md")
        assert "project/components.md" in content, "/retro must reference project/components.md for knowledge sync"

    def test_retro_updates_knowledge_docs(self):
        content = read("commands/retro.md")
        assert "components.md" in content, "/retro must reference components.md for knowledge update"
