...
class TestRetroGeneralPurposeAgent:
    def test_no_retro_format_plugin_agent(self):
        ...
        assert (
            ...
            and "subagent_type='zie-framework:retro-format'" not in text
    ...
    def test_no_general_purpose_agent(self):
        ...
        text = cmd_text()
        assert 'subagent_type="general-purpose"' not in text and "subagent_type='general-purpose'" not in text, (
...
class TestRetroAutoCommit:
    def test_auto_commit_present(self):
        ...
        text = cmd_text()
        assert "git commit" in text or "auto-commit" in text.lower() or "git add" in text, (
    ...
    def test_commit_message_format(self):
        ...
        text = cmd_text()
        assert "chore: retro" in text or "retro v" in text, "zie-retro commit message must use 'chore: retro' prefix"

    def test_push_on_failure_non_blocking(self):
        """Git push failure is non-blocking."""
        text = cmd_text()
        assert (
            "push failed" in text.lower()
            or "non-blocking" in text.lower()
            or "Manual push" in text
            or "manual push" in text.lower()
