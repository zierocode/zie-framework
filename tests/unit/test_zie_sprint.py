...
class TestAuditPhase:
    ...
    def test_audit_classifies_items(self):
        text = _text()
        assert "needs_spec" in text or "Needs Spec" in text, "AUDIT must classify items needing spec"

    def test_audit_asks_confirmation(self):
        text = _text()
        assert "yes" in text.lower() and "cancel" in text.lower(), "AUDIT must ask for confirmation before executing"
...
class TestPhaseStructure:
    ...
    def test_phase2_sequential_wip1(self):
        ...
        phase3_idx = text.index("PHASE 3")
        phase2_section = text[phase2_idx:phase3_idx]
        assert "sequential" in phase2_section.lower() or "WIP=1" in phase2_section, "Phase 2 must be sequential (WIP=1)"
    ...
    def test_phase3_batch_release(self):
        ...
        assert "release" in phase3_section.lower(), "Phase 3 must invoke release"

    def test_phase4_retro(self):
        text = _text()
        phase4_idx = text.index("PHASE 4")
        phase4_section = text[phase4_idx:]
        assert "retro" in phase4_section.lower(), "Phase 4 must invoke retro"
...
class TestContextBundle:
    def test_context_bundle_loaded_once(self):
        ...
        assert "context_bundle" in text, "context_bundle must be loaded (once) in the Load Context Bundle section"
    ...
    def test_context_bundle_referenced_in_phase1(self):
        ...
        phase2_idx = text.index("PHASE 2")
        phase1_section = text[phase1_idx:phase2_idx]
        assert "context_bundle" in phase1_section, "context_bundle must be referenced in PHASE 1 agent spawning"

    def test_adrs_loaded(self):
        assert "ADRs" in _text() or "decisions" in _text(), "context bundle must load ADRs/decisions"

    def test_context_md_loaded(self):
        assert "context" in _text().lower() and ("project" in _text().lower() or "context_bundle" in _text()), (
            "context bundle must load project context"
        )
...
class TestArgumentParsing:
    ...
    def test_slug_filtering(self):
        text = _text()
        assert "slugs" in text or "slug" in text.lower(), "must support filtering by specific slugs"


class TestErrorHandling:
    def test_empty_backlog_handling(self):
        text = _text()
        assert "error" in text.lower() or "Missing items" in text, "must handle empty/missing backlog items"

    def test_wip_active_handling(self):
        text = _text()
        assert "WIP" in text or "active" in text.lower(), "must warn when WIP item is active"

    def test_phase_failure_halts(self):
        text = _text()
        assert "halt" in text.lower() or "STOP" in text or "stop" in text.lower(), "phase failure must halt sprint"

    def test_phase4_retro_non_blocking(self):
        text = _text()
        assert "non-blocking" in text.lower() or "Non-blocking" in text, "Phase 4 retro failure must be non-blocking"


class TestSummaryOutput:
    def test_has_summary_section(self):
        assert "SPRINT COMPLETE" in _text() or "Summary" in _text(), "must print sprint summary on completion"
...
class TestPhase2ImplementAgent:
    def test_phase2_uses_zie_implement_make_target(self):
        ...
        assert "zie-implement" in phase2, "Sprint Phase 2 must call make zie-implement (agent mode via Bash)"
    ...
    def test_phase2_checks_roadmap_after_implement(self):
        ...
        phase3_idx = t.index("PHASE 3")
        phase2 = t[phase2_idx:phase3_idx]
        assert "ROADMAP" in phase2, "Phase 2 must check ROADMAP.md after implement to confirm success"
...
class TestAllItemsEnforcement:
    ...
    def test_all_means_all_language(self):
        text = _text()
        assert "ALL" in text or "all" in text.lower(), "must state that all items means every item"
    ...
    def test_consolidation_explains_original_items(self):
        text = _text()
        assert "slug-a" in text or "slug-b" in text or "both touch" in text, (
