...
class TestImplementAgentModeWarning:
    ...
    def test_recommend_agent_mode_command(self):
        ...
        text = cmd_text()
        assert "builder" in text, "zie-implement must reference zie-framework:builder"
    ...
    def test_advisory_is_non_blocking(self):
        ...
        text = cmd_text()
        assert "advisory" in text.lower() or "continue immediately" in text.lower() or "do not block" in text.lower(), (
    ...
    def test_normal_flow_unchanged(self):
        ...
        text = cmd_text()
        assert "RED" in text and "GREEN" in text and "REFACTOR" in text, "TDD flow must remain unchanged"
