...
class TestAuditProgress:
    ...
    def test_phase_structure_present(self):
        text = self._skill()
        assert "## Phase 1" in text and "## Phase 2" in text, "audit skill must contain Phase 1 and Phase 2 sections"

    def test_three_dimension_agents_present(self):
        text = self._skill()
        assert "Agent A" in text and "Agent B" in text, "audit skill must describe dimension agents (A, B,...)"
...
class TestReleaseProgress:
    def test_gate_1_counter_present(self):
        ...
        assert "Gate 1/5" in text, "release.md must contain Gate 1/5 counter"

    def test_gate_2_counter_present(self):
        text = (COMMANDS_DIR / "release.md").read_text()
        assert "Gate 2/5" in text, "release.md must contain Gate 2/5 counter"

    def test_step_counter_present(self):
        text = (COMMANDS_DIR / "release.md").read_text()
        assert "| Step |" in text or "| 1 |" in text, "release.md must contain post-gate step table"


class TestPlanProgress:
    def test_plan_counter_present(self):
        text = (COMMANDS_DIR / "plan.md").read_text()
        assert "[Plan " in text, "zie-plan.md must contain [Plan N/M] counter notation"

    def test_reviewer_pass_marker_present(self):
        text = (COMMANDS_DIR / "plan.md").read_text()
        assert "plan-review pass" in text, "zie-plan.md must contain 'plan-review pass' marker"


class TestResyncProgress:
    def test_exploring_marker_present(self):
        text = (COMMANDS_DIR / "resync.md").read_text()
        assert "Exploring codebase" in text, "zie-resync.md must contain 'Exploring codebase...' start marker"

    def test_explored_completion_marker_present(self):
        text = (COMMANDS_DIR / "resync.md").read_text()
        assert "Explored" in text, "zie-resync.md must contain 'Explored' completion marker"


class TestRetroProgress:
    def test_adr_counter_present(self):
        text = (COMMANDS_DIR / "retro.md").read_text()
        assert "[ADR " in text, "zie-retro.md must contain [ADR N/M] counter notation"

    def test_analyzing_git_log_marker_present(self):
        text = (COMMANDS_DIR / "retro.md").read_text()
        # v1.32.0: retro uses "Commits since last tag" as the git-log phase marker
        assert "Commits since last tag" in text or "Analyzing git log" in text, (
            "retro.md must contain git-log analysis phase marker"
        )

    def test_updating_knowledge_docs_marker_present(self):
        text = (COMMANDS_DIR / "retro.md").read_text()
        # v1.32.0: retro uses "อัปเดต project knowledge" as knowledge-update marker
        assert "อัปเดต project knowledge" in text or "Updating knowledge docs" in text, (
            "retro.md must contain knowledge docs update phase marker"
        )
