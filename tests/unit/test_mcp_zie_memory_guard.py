...
def test_backlog_brain_calls_guarded():
    ...
    assert not violations, (
        f"backlog.md: unguarded zie-memory calls at lines {violations} — "
        "wrap with if zie_memory_enabled=true"
    )
...
def test_plan_brain_calls_guarded():
    ...
    violations = _has_unguarded_mcp(text)
    assert not violations, (
        f"plan.md: unguarded zie-memory calls at lines {violations}"
    )
...
def test_implement_brain_calls_guarded():
    ...
    violations = _has_unguarded_mcp(text)
    assert not violations, (
        f"implement.md: unguarded zie-memory calls at lines {violations}"
    )
...
def test_retro_brain_calls_guarded():
    ...
    violations = _has_unguarded_mcp(text)
    assert not violations, (
        f"retro.md: unguarded zie-memory calls at lines {violations}"
    )
...
def test_fix_brain_calls_guarded():
    ...
    violations = _has_unguarded_mcp(text)
    assert not violations, (
        f"fix.md: unguarded zie-memory calls at lines {violations}"
    )
...
def test_release_brain_calls_guarded():
    ...
    violations = _has_unguarded_mcp(text)
    assert not violations, (
        f"release.md: unguarded zie-memory calls at lines {violations}"
    )
