"""Tests for context-lean-sprint Task 1: spec-review accepts context_bundle."""

from pathlib import Path

SKILL_PATH = Path(__file__).parents[2] / "skills" / "spec-review" / "SKILL.md"
...
class TestSpecReviewerContextBundle:
    def test_context_bundle_phase_present(self):
        """spec-review documents context_bundle parameter."""
        text = skill_text()
        assert "context_bundle" in text, "spec-review must document context_bundle parameter"
    ...
    def test_uses_bundle_adrs_when_provided(self):
        ...
        assert "context_bundle.adrs" in text or "context_bundle" in text, (
            "spec-review must use context_bundle.adrs when provided"
        )
    ...
    def test_uses_bundle_context_when_provided(self):
        ...
        assert "context_bundle.context" in text or "context_bundle" in text, (
            "spec-review must use context_bundle.context when provided"
        )
    ...
    def test_fallback_to_disk_when_no_bundle(self):
        ...
        text = skill_text()
        assert (
            "absent" in text.lower()
            or "fallback" in text.lower()
            or "backward-compatible" in text.lower()
            or "disk" in text.lower()
        ), "spec-review must document fallback to disk reads"

    def test_review_checklist_unchanged(self):
        """Phase 2 review checklist remains unchanged."""
        text = skill_text()
        assert "YAGNI" in text and "Testability" in text, "spec-review Phase 2 checklist must be unchanged"
