...
class TestZieAuditSharedContext:
    def test_shared_context_bundle_present(self):
        ...
        assert "shared_context" in skill_text(), "audit skill Phase 1 must build shared_context bundle"
    ...
    def test_no_redundant_manifest_reads(self):
        ...
        text = skill_text()
        assert "package.json" in text or "pyproject.toml" in text, "Manifests must be detected in Phase 1"
