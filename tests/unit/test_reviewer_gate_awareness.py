...
class TestSpecCommandDraftPlanApproval:
    ...
    def test_spec_allowed_tools_has_bash(self):
        ...
        assert "Bash" in frontmatter, "spec.md must have Bash in allowed-tools (needed for approve.py in --draft-plan)"
    ...
    def test_spec_draft_plan_uses_approve_py(self):
        ...
        assert "approve.py" in draft_section, "spec --draft-plan branch must use approve.py to approve plan"
...
class TestWritePlanApprovalGate:
    ...
    def test_write_plan_approve_py_path_documented(self):
        ...
        assert "approve.py" in _wp(), "write-plan must document python3 hooks/approve.py as the only approval path"

    def test_write_plan_caller_owns_approval(self):
        """write-plan must state approval is caller's responsibility."""
        t = _wp().lower()
        assert "caller" in t, "write-plan must state that approval (approve.py) is the caller's responsibility"
