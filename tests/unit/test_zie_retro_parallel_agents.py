...
class TestRetroInlineWrites:
    def test_adr_and_roadmap_both_present(self):
        ...
        text = cmd_text()
        assert "ADR" in text and "ROADMAP" in text, "zie-retro must have both ADR and ROADMAP update instructions"

    def test_no_run_in_background_in_retro(self):
        """Retro must not use run_in_background (inline writes, not agents)."""
        text = cmd_text()
        assert "run_in_background" not in text, "zie-retro must not use run_in_background for ADR/ROADMAP writes"
    ...
    def test_no_subagent_type_in_adr_section(self):
        ...
        text = cmd_text()
        assert "subagent_type=" not in text, "zie-retro must not use subagent_type in ADR/ROADMAP section"
