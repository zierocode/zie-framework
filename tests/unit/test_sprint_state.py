...
class TestSprintResumeCheck:
    def test_sprint_state_file_referenced(self):
        ...
        assert ".sprint-state" in _text(), "sprint.md must reference zie-framework/.sprint-state for resume/checkpoint"
    ...
    def test_resume_check_in_preflight(self):
        ...
        assert resume_idx < audit_idx, "Sprint resume check must come before Step 0 AUDIT"
...
class TestSprintStateWrites:
    def test_state_written_after_phase1(self):
        ...
        assert ".sprint-state" in phase1_section, "Sprint state must be written after Phase 1 completes"
    ...
    def test_state_written_after_phase2(self):
        ...
        assert ".sprint-state" in phase2_section, "Sprint state must be written after Phase 2 completes"
    ...
    def test_state_written_after_phase3(self):
        ...
        assert ".sprint-state" in phase3_section, "Sprint state must be written after Phase 3 completes"
...
class TestSprintStateCleanup:
    def test_state_deleted_after_retro(self):
        ...
        assert ".sprint-state" in phase4_section, "Phase 4 must reference .sprint-state in cleanup instruction"
...
class TestSprintStateEnrichment:
    ...
    def test_current_task_in_state(self):
        ...
        assert "current_task" in _text(), "sprint.md must include current_task in .sprint-state JSON"

    def test_last_action_in_state(self):
        """Sprint state must include last_action field."""
        assert "last_action" in _text(), "sprint.md must include last_action in .sprint-state JSON"

    def test_impl_done_action(self):
        """impl-done must set last_action."""
        assert "impl-done" in _text(), "sprint.md must set last_action to impl-done when implementation completes"

    def test_compact_between_items(self):
        """Compact between items must be referenced in Phase 2."""
        text = _text()
        assert "compact" in text.lower() and "context cleared" in text, (
            "sprint.md must reference /compact and context cleared between Phase 2 items"
        )

    def test_phase3_state_write(self):
        """Phase 3 must write .sprint-state with phase=3."""
        text = _text()
        phase3_idx = text.index("PHASE 3")
        phase4_idx = text.index("PHASE 4")
        phase3_section = text[phase3_idx:phase4_idx]
        assert "phase=3" in phase3_section or ".sprint-state" in phase3_section, (
            "sprint.md must write .sprint-state with phase=3 in Phase 3"
        )
...
class TestImplementTddPhase:
    ...
    def test_tdd_phase_write_mentioned(self):
        ...
        assert "tdd_phase" in text, "implement.md must reference tdd_phase for .sprint-state writes"

    def test_sprint_state_referenced(self):
        """implement.md must reference .sprint-state."""
        text = (REPO_ROOT / "commands" / "implement.md").read_text()
        assert ".sprint-state" in text, "implement.md must reference .sprint-state file"
