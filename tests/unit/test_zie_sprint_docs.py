...
class TestClaudeMdDocumentation:
    def test_zie_sprint_in_claude_md(self):
        assert "/sprint" in _claude_md(), "CLAUDE.md must mention /sprint"
...
class TestZieStatusSuggestions:
    def test_zie_sprint_in_status_suggestions(self):
        assert "/sprint" in _status_cmd(), "zie-status.md must include /sprint in always-available suggestions"
