...
def test_claudemd_line_count():
    ...
    assert len(lines) <= 120, (
        f"CLAUDE.md has {len(lines)} lines (limit: 120). Move reference material to zie-framework/project/ spoke docs."
    )
...
def test_claudemd_references_hook_conventions():
    content = (REPO_ROOT / "CLAUDE.md").read_text()
    assert "hook-conventions.md" in content, "CLAUDE.md trigger table must reference hook-conventions.md"


def test_claudemd_references_config_reference():
    content = (REPO_ROOT / "CLAUDE.md").read_text()
    assert "config-reference.md" in content, "CLAUDE.md trigger table must reference config-reference.md"
