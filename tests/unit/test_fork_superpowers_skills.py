...
class TestSkillFilesExist:
    def test_spec_design_skill_exists(self):
        assert os.path.exists(skill_path("spec-design")), "skills/spec-design/SKILL.md must exist"

    def test_write_plan_skill_exists(self):
        assert os.path.exists(skill_path("write-plan")), "skills/write-plan/SKILL.md must exist"

    def test_debug_skill_exists(self):
        assert os.path.exists(skill_path("debug")), "skills/debug/SKILL.md must exist"

    def test_verify_skill_exists(self):
        assert os.path.exists(skill_path("verify")), "skills/verify/SKILL.md must exist"
...
class TestSkillZieMemoryIntegration:
    ...
    def test_spec_design_saves_to_zie_framework_specs(self):
        content = read("skills/spec-design/SKILL.md")
        assert "zie-framework/specs/" in content, "spec-design skill must save specs to zie-framework/specs/"
        assert "docs/superpowers/specs/" not in content, "spec-design skill must not reference superpowers spec path"

    def test_write_plan_saves_to_zie_framework_plans(self):
        content = read("skills/write-plan/SKILL.md")
        assert "zie-framework/plans/" in content, "write-plan skill must save plans to zie-framework/plans/"
        assert "docs/superpowers/plans/" not in content, "write-plan skill must not reference superpowers plan path"
    ...
    def test_write_plan_has_depends_on_docs(self):
        content = read("skills/write-plan/SKILL.md")
        assert "depends_on" in content, "write-plan skill must document depends_on task marker syntax"
    ...
    def test_debug_no_superpowers_refs(self):
        content = read("skills/debug/SKILL.md")
        assert "superpowers:" not in content, "debug skill must not reference superpowers: skill names"

    def test_spec_design_no_superpowers_refs(self):
        content = read("skills/spec-design/SKILL.md")
        assert "superpowers:" not in content, "spec-design skill must not reference superpowers: skill names"

    def test_write_plan_no_superpowers_refs(self):
        content = read("skills/write-plan/SKILL.md")
        assert "superpowers:" not in content, "write-plan skill must not reference superpowers: skill names"


class TestCommandsNoSuperpowersDependency:
    def test_zie_spec_no_superpowers_skill(self):
        content = read("commands/spec.md")
        assert "Skill(superpowers:" not in content, "zie-spec must not call Skill(superpowers:*) after fork"

    def test_zie_implement_no_superpowers_skill(self):
        content = read("commands/implement.md")
        assert "Skill(superpowers:" not in content, "zie-implement must not call Skill(superpowers:*) after fork"

    def test_zie_fix_no_superpowers_skill(self):
        content = read("commands/fix.md")
        assert "Skill(superpowers:" not in content, "zie-fix must not call Skill(superpowers:*) after fork"

    def test_zie_release_no_superpowers_skill(self):
        content = read("commands/release.md")
        assert "Skill(superpowers:" not in content, "zie-release must not call Skill(superpowers:*) after fork"

    def test_zie_spec_calls_zie_framework_spec_design(self):
        content = read("commands/spec.md")
        assert "Skill(zie-framework:spec-design)" in content, "zie-spec must invoke Skill(zie-framework:spec-design)"
    ...
    def test_spec_design_does_not_auto_invoke_write_plan(self):
        ...
        assert "/plan" in content, "spec-design must print handoff to /plan instead of invoking write-plan"

    def test_zie_implement_calls_zie_framework_tdd_loop(self):
        content = read("commands/implement.md")
        assert "Skill(zie-framework:tdd-loop)" in content, "zie-implement must invoke Skill(zie-framework:tdd-loop)"

    def test_zie_implement_calls_zie_framework_debug(self):
        content = read("commands/implement.md")
        assert "Skill(zie-framework:debug)" in content, "zie-implement must invoke Skill(zie-framework:debug)"

    def test_zie_fix_calls_zie_framework_debug(self):
        content = read("commands/fix.md")
        assert "Skill(zie-framework:debug)" in content, "zie-fix must invoke Skill(zie-framework:debug)"

    def test_zie_fix_calls_zie_framework_verify(self):
        content = read("commands/fix.md")
        assert "Skill(zie-framework:verify)" in content, "zie-fix must invoke Skill(zie-framework:verify)"
    ...
    def test_zie_plan_no_superpowers_enabled(self):
        ...
        assert "superpowers_enabled" not in content, "zie-plan must not read superpowers_enabled from .config"
    ...
    def test_session_resume_no_superpowers_enabled(self):
        ...
        assert "superpowers_enabled" not in content, "session-resume hook must not read superpowers_enabled from config"
