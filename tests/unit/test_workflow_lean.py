...
class TestZieAuditFocusFlag:
    def test_focus_flag_documented(self):
        text = read("commands/zie-audit.md")
...
class TestZieSpecDraftPlanFlag:
    def test_draft_plan_flag_documented(self):
        text = read("commands/zie-spec.md")
    ...
    def test_no_flag_normal_handoff(self):
        text = read("commands/zie-spec.md")
        assert "/zie-plan" in text and "Next:" in text, \
            "zie-spec without --draft-plan must print normal handoff"

    def test_flag_removed_from_slug_extraction(self):
        text = read("commands/zie-spec.md")
        assert "clean_args" in text or "remove" in text.lower() or \
               '!= "--draft-plan"' in text, \
...
class TestSpecDesignArgsTable:
    def test_three_argument_rows(self):
        ...
        assert "$ARGUMENTS[2]" in text, \
            "spec-design Arguments table must document position 2"

    def test_draft_plan_documented(self):
        text = read("skills/spec-design/SKILL.md")
        assert "--draft-plan" in text, \
            "spec-design Arguments table must reference --draft-plan"
...
class TestZieInitSectionLoop:
    def test_section_targeted_prompt_present(self):
        text = read("commands/zie-init.md")
    ...
    def test_all_good_exits_loop(self):
        text = read("commands/zie-init.md")
        assert "all good" in text.lower(), \
            "zie-init must accept 'all good' to exit revision loop"

    def test_other_sections_retain_state(self):
        text = read("commands/zie-init.md")
        assert "retain" in text.lower() or "other" in text.lower() or \
               "prior state" in text.lower(), \
