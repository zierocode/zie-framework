...
class TestZieAuditSkill:
    ...
    def test_skill_file_exists(self):
        assert self.SKILL_PATH.exists(), \
            "skills/audit/SKILL.md must exist"

    def test_reference_file_exists(self):
        assert self.REF_PATH.exists(), \
            "skills/audit/reference.md must exist"

    def test_skill_under_500_lines(self):
        lines = self.SKILL_PATH.read_text().splitlines()
        assert len(lines) < 500, \
            f"skills/audit/SKILL.md must be under 500 lines (got {len(lines)})"

    def test_skill_has_argument_hint(self):
        content = self.SKILL_PATH.read_text()
        assert "argument-hint:" in content, \
            "skills/audit/SKILL.md must have argument-hint: in frontmatter"

    def test_skill_argument_hint_includes_focus(self):
        content = self.SKILL_PATH.read_text()
        assert "--focus" in content, \
            "skills/audit/SKILL.md argument-hint must document --focus flag"
    ...
    def test_skill_covers_all_9_dimensions(self):
        ...
        for dim in dimensions:
            assert dim in content, \
                f"skills/audit/SKILL.md must reference the {dim} dimension"

    def test_skill_reads_reference_md_via_skill_dir(self):
        content = self.SKILL_PATH.read_text()
        assert "CLAUDE_SKILL_DIR" in content, \
            "skills/audit/SKILL.md must reference ${CLAUDE_SKILL_DIR}/reference.md"
        assert "reference.md" in content, \
            "skills/audit/SKILL.md must explicitly read reference.md"
    ...
    def test_zie_audit_skill_has_phases(self):
        ...
        assert "Phase 1" in content and "Phase 4" in content, \
            "skills/audit/SKILL.md must have at least 4 phases"
...
class TestSkillDirReferencePattern:
    ...
    def test_zie_audit_reference_md_is_loaded_explicitly(self):
        ...
        content = read_skill("audit")
        assert "reference.md" in content, \
            "skills/audit/SKILL.md must explicitly name reference.md in its steps"
