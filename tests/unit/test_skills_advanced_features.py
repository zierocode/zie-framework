...
audit skill + reference.md supporting file, SKILL.md size guard.
...
class TestArgumentIndexedDocs:
    def test_spec_design_has_argument_hint_frontmatter(self):
        ...
        assert "argument-hint:" in content, "spec-design/SKILL.md must have argument-hint: in frontmatter"

    def test_spec_design_documents_arguments_0_as_slug(self):
        content = read_skill("spec-design")
        assert "$ARGUMENTS[0]" in content, "spec-design/SKILL.md must document $ARGUMENTS[0] as slug"

    def test_spec_design_documents_arguments_1_as_mode(self):
        content = read_skill("spec-design")
        assert "$ARGUMENTS[1]" in content, "spec-design/SKILL.md must document $ARGUMENTS[1] as mode"

    def test_spec_design_documents_mode_values(self):
        content = read_skill("spec-design")
        assert "full" in content and "quick" in content, "spec-design/SKILL.md must document full and quick mode values"
    ...
    def test_write_plan_has_argument_hint_frontmatter(self):
        ...
        assert "argument-hint:" in content, "write-plan/SKILL.md must have argument-hint: in frontmatter"

    def test_write_plan_documents_arguments_0_as_slug(self):
        content = read_skill("write-plan")
        assert "$ARGUMENTS[0]" in content, "write-plan/SKILL.md must document $ARGUMENTS[0] as slug"

    def test_write_plan_documents_arguments_1_as_flags(self):
        content = read_skill("write-plan")
        assert "$ARGUMENTS[1]" in content, "write-plan/SKILL.md must document $ARGUMENTS[1] as optional flags"
...
class TestArgumentHintFrontmatter:
    ...
    def test_no_arg_skills_hint_is_empty_string(self):
        ...
        for skill in self.NO_ARG_SKILLS:
            content = read_skill(skill)
            assert 'argument-hint: ""' in content or "argument-hint: ''" in content or "argument-hint:\n" in content, (
...
class TestZieAuditSkill:
    SKILL_PATH = SKILLS / "audit" / "SKILL.md"
    REF_PATH = SKILLS / "audit" / "reference.md"

    def test_skill_file_exists(self):
        assert self.SKILL_PATH.exists(), "skills/audit/SKILL.md must exist"

    def test_reference_file_exists(self):
        assert self.REF_PATH.exists(), "skills/audit/reference.md must exist"

    def test_skill_under_500_lines(self):
        lines = self.SKILL_PATH.read_text().splitlines()
        assert len(lines) < 500, f"skills/audit/SKILL.md must be under 500 lines (got {len(lines)})"

    def test_skill_has_argument_hint(self):
        content = self.SKILL_PATH.read_text()
        assert "argument-hint:" in content, "skills/audit/SKILL.md must have argument-hint: in frontmatter"

    def test_skill_argument_hint_includes_focus(self):
        content = self.SKILL_PATH.read_text()
        assert "--focus" in content, "skills/audit/SKILL.md argument-hint must document --focus flag"

    def test_skill_covers_all_9_dimensions(self):
        content = self.SKILL_PATH.read_text()
        # Skill references dimension names via Agent scope labels
        dimensions = [
            "Security",
            "Lean",
            "Quality",
        ...
        for dim in dimensions:
            assert dim in content, f"skills/audit/SKILL.md must reference the {dim} dimension"

    def test_skill_reads_reference_md_via_skill_dir(self):
        content = self.SKILL_PATH.read_text()
        assert "CLAUDE_SKILL_DIR" in content, "skills/audit/SKILL.md must reference ${CLAUDE_SKILL_DIR}/reference.md"
        assert "reference.md" in content, "skills/audit/SKILL.md must explicitly read reference.md"

    def test_skill_has_graceful_skip_for_reference(self):
        content = self.SKILL_PATH.read_text()
        assert "graceful" in content.lower() or "not found" in content.lower() or "skip" in content.lower(), (
            "skills/audit/SKILL.md must document graceful skip if reference.md is missing"
        )

    def test_reference_has_scoring_rubric(self):
        content = self.REF_PATH.read_text()
        assert "100" in content, "reference.md must document the start-at-100 scoring system"
    ...
    def test_reference_has_dimension_definitions(self):
        ...
        for dim in dimensions:
            assert dim in content, f"reference.md must define the {dim} dimension"
    ...
    def test_zie_audit_skill_has_phases(self):
        """audit skill must have multi-phase structure (canonical since lean-dual-audit-pipeline)."""
        content = read_skill("audit")
        assert "Phase 1" in content and "Phase 4" in content, "skills/audit/SKILL.md must have at least 4 phases"
...
class TestSkillFileSizeGuard:
    def test_no_skill_md_exceeds_500_lines(self):
        ...
        assert not oversized, "These SKILL.md files exceed 500 lines — extract reference content: " + ", ".join(
            oversized
        )
...
class TestSkillDirReferencePattern:
    def test_zie_audit_uses_claude_skill_dir(self):
        ...
        assert "${CLAUDE_SKILL_DIR}" in content, (
            "skills/audit/SKILL.md must use ${CLAUDE_SKILL_DIR} to reference supporting files"
        )
    ...
    def test_zie_audit_reference_md_is_loaded_explicitly(self):
        ...
        content = read_skill("audit")
        assert "reference.md" in content, "skills/audit/SKILL.md must explicitly name reference.md in its steps"
