...
class TestParallelReleaseGates:
    def test_docs_sync_spawns_before_gate1(self):
        ...
        assert pre_gate1_idx > 0 and gate1_idx > pre_gate1_idx, "Pre-Gate-1 docs-sync section must appear before Gate 1"
    ...
    def test_all_gate_failures_collected_before_stopping(self):
        ...
        text = cmd_text()
        assert "Collect all results" in text or "collect" in text.lower(), (
            "All gate results must be collected before stopping"
        )
        assert "Any fail" in text or "Gates failed" in text, "Must collect all gate results before stopping on failure"

    def test_gate_bash_calls_present(self):
        """Gates 2, 3, 4 use inline Bash calls (no Agent() spawning)."""
        text = cmd_text()
        assert "Agent(" not in text or text.count("Agent(") == 0, "Gates must use inline Bash, not Agent() spawning"
