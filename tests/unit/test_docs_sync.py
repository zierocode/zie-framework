...
class TestMakefileBumpCallsSyncVersion:
    def test_bump_target_calls_bump_extra(self):
        ...
        assert "_bump-extra" in bump_block, "bump target does not call _bump-extra"
...
class TestProjectMd:
    ...
    def test_commands_table_header_is_english(self):
        ...
        assert "ทำอะไร" not in content, "PROJECT.md still contains Thai header 'ทำอะไร'"
        assert "| Command | Description |" in content, "Commands table header 'Description' not found"
...
class TestArchitectureMd:
    ...
    def test_v1_5_0_mentions_knowledge_hash(self):
        ...
        assert "knowledge-hash.py" in self._content(), "v1.5.0 entry should mention knowledge-hash.py"
...
class TestComponentsMd:
    ...
    def test_knowledge_hash_entry_exists(self):
        assert "knowledge-hash.py" in self._content(), "knowledge-hash.py not documented in components.md"
        ...
        for line in content.splitlines():
            ...
            if in_hooks and "knowledge-hash.py" in line:
                raise AssertionError(
                    "knowledge-hash.py is listed inside the Hooks section — it should be in a Utility Scripts section"
                )

    def test_utility_scripts_section_exists(self):
        assert "Utility Scripts" in self._content(), "No 'Utility Scripts' section found in components.md"
...
class TestClaudeMd:
    ...
    def test_optional_deps_documents_playwright(self):
        assert "playwright" in self._content(), "CLAUDE.md must document playwright optional dependency"
    ...
    def test_sync_version_in_dev_commands(self):
        assert "sync-version" in self._content(), "CLAUDE.md Development Commands missing 'make sync-version'"
...
class TestReadmeMd:
    ...
    def test_skills_section_exists(self):
        assert "## Skills" in self._content(), "README.md missing ## Skills section"

    def test_skills_table_has_tdd_loop(self):
        assert "tdd-loop" in self._content(), "Skills table missing tdd-loop entry"

    def test_skills_table_has_zie_audit(self):
        assert "audit" in self._content(), "Skills table missing audit entry"
