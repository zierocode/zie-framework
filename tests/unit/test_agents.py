...
class TestImplementModeAgent:
    def test_file_exists(self):
        assert (AGENTS_DIR / "builder.md").exists(), "agents/implement-mode.md not found"

    def test_frontmatter_model(self):
        fm, _ = parse_agent_file("builder.md")
        assert fm.get("model", "").split("#")[0].strip() == "sonnet", f"Expected model: sonnet, got: {fm.get('model')}"
    ...
    def test_frontmatter_tools_all(self):
        fm, _ = parse_agent_file("builder.md")
        assert fm.get("tools") == "all", f"Expected tools: all, got: {fm.get('tools')}"
    ...
    def test_body_mentions_wip_rule(self):
        _, body = parse_agent_file("builder.md")
        assert "WIP=1" in body or "WIP = 1" in body, "System prompt must mention WIP=1 rule"
...
class TestAuditModeAgent:
    def test_file_exists(self):
        assert (AGENTS_DIR / "auditor.md").exists(), "agents/audit-mode.md not found"
    ...
    def test_frontmatter_no_tools_restriction(self):
        ...
        assert fm.get("tools") is None, f"tools should be absent (audit needs full tool access), got: {fm.get('tools')}"

    def test_frontmatter_tools_excludes_write(self):
        fm, _ = parse_agent_file("auditor.md")
        tools_val = fm.get("tools", "")
        tool_list = re.split(r"[\[\],\s]+", tools_val)
        assert "Write" not in tool_list, f"Write tool must not be in audit-mode tool list: {tools_val}"
    ...
    def test_body_mentions_audit_mode_message(self):
        _, body = parse_agent_file("auditor.md")
        assert "audit mode" in body.lower(), "System prompt must mention 'audit mode' for user-facing messaging"

    def test_body_directs_findings_to_backlog(self):
        _, body = parse_agent_file("auditor.md")
        assert "backlog" in body.lower(), "System prompt must instruct surfacing findings as backlog candidates"
...
class TestPluginJsonAgentsDir:
    def test_agents_dir_key_present(self):
        ...
        assert "agentsDir" in plugin, "plugin.json must contain 'agentsDir' key"

    def test_agents_dir_value(self):
        plugin = json.loads((REPO_ROOT / ".claude-plugin" / "plugin.json").read_text())
        assert plugin["agentsDir"] == "agents", f"Expected agentsDir: 'agents', got: {plugin.get('agentsDir')}"


class TestSettingsJson:
    def test_settings_json_exists(self):
        assert (REPO_ROOT / "settings.json").exists(), "settings.json not found at plugin root"

    def test_settings_json_has_default_agent(self):
        settings = json.loads((REPO_ROOT / "settings.json").read_text())
        assert "defaultAgent" in settings, "settings.json must contain 'defaultAgent' key"
    ...
    def test_settings_json_has_invocation_key(self):
        settings = json.loads((REPO_ROOT / "settings.json").read_text())
        assert "invocation" in settings, "settings.json must contain 'invocation' key documenting usage examples"
...
class TestReadmeAgentDocs:
    def test_agent_modes_section_present(self):
        ...
        assert "Agent Modes" in readme, "README.md must contain an 'Agent Modes' section"

    def test_implement_mode_in_readme(self):
        readme = (REPO_ROOT / "README.md").read_text()
        assert "builder" in readme, "README.md must document builder agent"

    def test_audit_mode_in_readme(self):
        readme = (REPO_ROOT / "README.md").read_text()
        assert "auditor" in readme, "README.md must document auditor agent"

    def test_agent_invocation_command_in_readme(self):
        readme = (REPO_ROOT / "README.md").read_text()
        assert "--agent" in readme, "README.md must show --agent flag in invocation example"


class TestClaudeMdAgentDocs:
    def test_agent_invocation_in_claude_md(self):
        claude_md = (REPO_ROOT / "CLAUDE.md").read_text()
        assert "--agent" in claude_md, "CLAUDE.md must include --agent invocation example"

    def test_implement_mode_in_claude_md(self):
        claude_md = (REPO_ROOT / "CLAUDE.md").read_text()
        assert "builder" in claude_md, "CLAUDE.md must reference builder"
...
class TestComponentsRegistryAgents:
    def test_agents_section_present(self):
        ...
        assert "## Agents" in components, "components.md must have an '## Agents' section"

    def test_implement_mode_in_components(self):
        components = (REPO_ROOT / "zie-framework" / "project" / "components.md").read_text()
        assert "builder" in components, "components.md Agents section must list builder"

    def test_audit_mode_in_components(self):
        components = (REPO_ROOT / "zie-framework" / "project" / "components.md").read_text()
        assert "auditor" in components, "components.md Agents section must list auditor"
