...
def test_zie_implement_task_counter_header(zie_implement_md):
    assert "[T{N}/{total}]" in zie_implement_md, "zie-implement must print [T{N}/{total}] before each task"


def test_zie_implement_phase_markers(zie_implement_md):
    for marker in ["→ RED", "→ GREEN", "→ REFACTOR"]:
        assert marker in zie_implement_md, f"zie-implement must print phase marker: {marker}"
...
def test_zie_audit_phase_header(zie_audit_md):
    assert "[Phase" in zie_audit_md, "audit must print [Phase N/M] headers"


def test_zie_audit_agent_completion_marker(zie_audit_md):
    assert "Agent" in zie_audit_md and "✓" in zie_audit_md, "audit must print Agent {X} (Domain) ✓ per spawned agent"


def test_zie_audit_research_counter(zie_audit_md):
    assert "[Research" in zie_audit_md, "audit must print [Research {N}/15] per search call"
...
def test_zie_resync_bracketed_start(zie_resync_md):
    assert "[Exploring codebase...]" in zie_resync_md, "zie-resync must print [Exploring codebase...] (bracketed)"
...
def test_zie_sprint_task_create_per_phase(zie_sprint_md):
    assert zie_sprint_md.count("TaskCreate") >= 5, "zie-sprint must call TaskCreate for each of 5 phases"


def test_zie_sprint_task_update_per_phase(zie_sprint_md):
    assert zie_sprint_md.count("TaskUpdate") >= 5, "zie-sprint must call TaskUpdate to mark each phase complete"


def test_zie_sprint_progress_bar(zie_sprint_md):
    assert "████" in zie_sprint_md or "{done}/{total}" in zie_sprint_md, "zie-sprint must print Unicode progress bar"


def test_zie_sprint_eta_signal(zie_sprint_md):
    assert "phases remaining" in zie_sprint_md, "zie-sprint must print phase-count ETA signal"
