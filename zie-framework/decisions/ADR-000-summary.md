...
| ADR-003 | Commands Are Control Plane, Skills Are Execution | Commands own pipeline transitions; skills execute within one stage, never auto-advance | Accepted |
| ADR-004 | Spec Approval State Tracked via Frontmatter | spec-review approval → approved: true frontmatter; /plan filters by flag | Accepted |
| ADR-005 | Hybrid Release — SDLC Gates + Project Publish | Release: SDLC layer (gates + bump) + project layer (git ops + publish) | Accepted |
...
| ADR-010 | safe_write_tmp Hard-Fails on Symlink | checks is_symlink() before writing; logs WARNING, never raises | Accepted |
| ADR-011 | find_matching_test OSError Guards | try/except OSError at each filesystem call | Accepted |
| ADR-012 | Tiered Model Routing — haiku / sonnet / opus | Three-tier routing: opus+high for audit, sonnet+high/medium for design/plan/impl, haiku+low for checklist/reviewer tasks | Accepted |
| ADR-013 | Plugin-Bundled MCP Server for Zero-Setup Brain Integration | Ship .claude-plugin/.mcp.json to auto-register zie-memory MCP; degrades gracefully when env vars absent | Accepted |
| ADR-014 | Async impl-review Deferred-Check Polling | Spawn impl-review async after REFACTOR; poll at next iteration; surface issues; max 2 iterations | Accepted |
...
| ADR-017 | impl-review Upgraded from haiku/low to sonnet/medium | impl-review → sonnet/medium; spec-review and plan-review remain haiku | Accepted |
...
| ADR-021 | audit Cost Optimization | Replace 5 Opus agents with 3 Sonnet agents + synthesis; ~72% cost reduction; WebSearch 25 → 15 | Accepted |
...
| ADR-032 | Shared Context Bundle in audit | Build context bundle once in Phase 1; pass to all parallel Phase 2 agents. | Accepted |
...
| ADR-058 | Inline Reviewer Replaces Async Agent | impl-review: async Agent → inline Skill(); HIGH risk only; 1 auto-fix retry. | Accepted |
...
| ADR-066 | Non-Claude Model Compatibility | Env var model resolution; regex fallback when subagent model unavailable | Accepted |
| ADR-067 | Release Skill Git Ops Direct | /release performs git ops directly; make _publish hook for project publish | Accepted |
| — | v1.29.0 | Auto-learn/decide/improve, unified-context-cache, light-retro, WIP=1 impl | Accepted |
