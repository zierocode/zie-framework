...
- `skills/plan-reviewer/SKILL.md` — no change (already inline)
- `skills/spec-reviewer/SKILL.md` — no change (already inline)
**Data Flow:**

*Current flow (sequential Skill calls within single context):*
1. For each item in needs_spec: invoke spec-design Skill → wait → invoke write-plan Skill → wait → invoke plan-reviewer Skill → wait → approve
...
2. For each item in needs_spec: spawn Agent with `subagent_type: "general-purpose"` and `run_in_background: true`
   - Each agent runs the full pipeline: spec-design (autonomous, runs spec-reviewer inline) → write-plan → plan-reviewer (inline) → approve.py (marks spec+plan as approved)
...
- **Inline reviewer compatibility:** Each agent runs its own spec-reviewer and plan-reviewer inline (ADR-054/ADR-058). No conflict between items' reviewers — each agent has its own context.
